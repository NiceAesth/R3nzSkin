...
static inline void testFunc() noexcept {
  // This function is called upon pressing F7 ingame.
  ...
  for (auto i{0u}; i < minions->length; ++i) {
    ...
    cheatManager.logger->addLog(
      "Minion: %s\n\tModelName: %s\n\t", minion->get_name()->c_str(),
      minion->get_character_data_stack()->base_skin.model.str
    );
    if (owner) {
      cheatManager.logger->addLog(
        "OwnerName: %s\n\t\tModelName: %s\n\t", owner->get_name()->c_str(),
        owner->get_character_data_stack()->base_skin.model.str
      );
    }
...
static void init_imgui(void *device, bool is_d3d11 = false) noexcept {
  ...
  originalWndProc =
    WNDPROC(::SetWindowLongPtr(cheatManager.memory->window, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(&wndProc)));
...
void Hooks::init() noexcept {
  ...
  const auto minions{cheatManager.memory->minionList};
  static const auto playerHash{
    player ? fnv::hash_runtime(player->get_character_data_stack()->base_skin.model.str) : 0u};
  ...
  for (auto i{0u}; i < heroes->length; ++i) {
    if (const auto hero{heroes->list[i]}; !hero->get_character_data_stack()->stack.empty()) {
      // Viego transforms into another champion as 2nd form, our own skin's id may not match for every champion. (same
      // problem exists in sylas)
      if (const auto championName{fnv::hash_runtime(hero->get_character_data_stack()->base_skin.model.str)};
          championName == FNV("Viego") || championName == FNV("Sylas")) {
        continue;
      }

      if (auto &stack{hero->get_character_data_stack()->stack.front()};
  ...
  for (auto i{0u}; i < minions->length; ++i) {
    ...
    if (const auto config_entry{cheatManager.config->current_combo_jungle_mob_skin_index.find(hash)};
        config_entry != cheatManager.config->current_combo_jungle_mob_skin_index.end() && config_entry->second != 0) {
