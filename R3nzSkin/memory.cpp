...
[[nodiscard]] static std::uint8_t *find_signature(const wchar_t *szModule, const char *szSignature) noexcept {
  ...
  const auto ntHeaders{
    reinterpret_cast<PIMAGE_NT_HEADERS>(reinterpret_cast<std::uint8_t *>(module) + dosHeader->e_lfanew)};
    ...
    for (auto j{0ul}; j < s; ++j) {
      ...
      if (current_address >= next_check_address) {
        ...
        if (mbi.Protect == PAGE_NOACCESS) {
          i += reinterpret_cast<std::uintptr_t>(mbi.BaseAddress) + mbi.RegionSize -
               (reinterpret_cast<std::uintptr_t>(scanBytes) + i);
          i--;
          found = false;
          break;
        } else {
...
void Memory::update(bool gameClient) noexcept {
  if (gameClient) {
    ...
    this->heroList =
      *reinterpret_cast<ManagerTemplate<AIHero> **>(this->base + offsets::global::ManagerTemplate_AIHero_);
    this->minionList = *reinterpret_cast<ManagerTemplate<AIMinionClient> **>(
      this->base + offsets::global::ManagerTemplate_AIMinionClient_
    );
    this->turretList =
      *reinterpret_cast<ManagerTemplate<AITurret> **>(this->base + offsets::global::ManagerTemplate_AITurret_);
    this->championManager  = *reinterpret_cast<ChampionManager **>(this->base + offsets::global::ChampionManager);
    this->materialRegistry = reinterpret_cast<std::uintptr_t(__fastcall *)()>(
      this->base + offsets::functions::Riot__Renderer__MaterialRegistry__GetSingletonPtr
    )();
    this->d3dDevice =
      *reinterpret_cast<IDirect3DDevice9 **>(this->materialRegistry + offsets::MaterialRegistry::D3DDevice);
    this->swapChain =
      *reinterpret_cast<IDXGISwapChain **>(this->materialRegistry + offsets::MaterialRegistry::SwapChain);
    this->window = *reinterpret_cast<HWND *>(this->base + offsets::global::Riot__g_window);
    this->translateString =
      reinterpret_cast<translateString_t>(this->base + offsets::functions::translateString_UNSAFE_DONOTUSE);
  }
}
